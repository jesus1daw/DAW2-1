...
//-------EJERCICIO_1------------\\
//BOTON "importar", evento "click" MODIFICADO
document.getElementById('importar-boton').addEventListener('click', async () => {
    ...
    if (!archivoLibros || !archivoLectores || !archivoPrestamos) {
        texto="Error: Selecciona todos los archivos antes de importar.";
        color="red"
    ...
    //Sino leemos los dos CSV
        ...
        const lineas3 = contenidoPrestamos.split('\r\n');
        sinDuplicados=new Set(lineas3);
        sinDuplicados.delete("");
        arraySin=[...sinDuplicados];
        console.log(sinDuplicados);

        //Separamos los campos y creamos objetos Prestamo de cada linea
        sinDuplicados.forEach(linea => {
            let dato = linea.split(",");
            if (dato[0] != "numPrestamo") {
                let prestamo = new Prestamo(dato[0], dato[1], dato[2], dato[3], dato[4]);
                arrayTotalPrestamos .push(prestamo); //Los metemos en el array de Prestamos
    ...
    document.getElementById('importar').appendChild(mensajeDiv);
});



//-----------------EJERCICIO_2-----------\\
//Evento mouseenter que cuando entra a vista-libros se situa en el parrafo cursor-sobre-vista-libros y pone el texto de entró a vista libros
document.getElementById('vista-libros').addEventListener('mouseenter',() => {

let parrafo=document.getElementById("cursor-sobre-vista-libros");
    parrafo.textContent="Entró en vista libros";
});
//Evento mouseleave que cuando sale de vista-libros se situa en el parrafo cursor-sobre-vista-libros y pone el texto de Salió de vista libros
document.getElementById('vista-libros').addEventListener('mouseleave',() => {

    let parrafo=document.getElementById("cursor-sobre-vista-libros");
        parrafo.textContent="Salio de vista libros";
});
...
let parrafoNav=document.getElementById('propiedades-navegador');
parrafoNav.innerHTML="Codigo del lenguaje: "+navigator.language+"<br>"+ 
"Alto de la pantalla:"+window.innerHeight+"Pixeles <br>"+
"Ancho de la pantalla"+window.innerWidth+"Pixeles";


//-----------------EJERCICIO_4-----------\\
//FUNCIÓN para validar el teleéfono MODIFICADA CON +34 opcional
function validarTelefono(telefono) {
    return /^(?:\+34\s?)?\d{9}$/.test(telefono);
}


//-----------------EJERCICIO_5-----------\\
function crearNuevaListaLibros(){
    nuevoObjetoListaLibros=new Object();

    arrayLibros.forEach(libro => {
        nuevoObjetoListaLibros.libros=(libro);
    });
...
//-----------------EJERCICIO_6-----------\\
//FUNCIÓN para actualizar las tablas de libros MODIFICADA con el filtro
function actualizarTablaLibros() {
    ...
    const filtro=document.getElementById('vista-libros-incluye').value;
    //Por cada libro creamos una fila
    arrayLibros.forEach(libro => {

        if(libro.titulo.includes(filtro)){ //Filtramos por la cadena de texto que nos han introducido
...
//-----------------EJERCICIO_7-----------\\
//FUNCION que comprueba el Libro con promesas
function comprobarLibro(valor){

    let miPromesa = new Promise((resolve, reject) => {
        let exito = true; // Simulación de éxito o error
        arrayLibros.forEach(libro => {

            //Si se encentra relacion y el libro no esta de baja y mas de 0 ejemplares
            if(libro.codLibro==valor && libro.bajaLibro==false && libro.ejemplares>0){
                exito=false;
            }
        });
          if (exito) {
            ...
            reject("Libro (inexistente, dado de baja o sin ejemplares)"); // Promesa rechazada
          }
      });
      return miPromesa;
}

//FUNCION que comprueba el Lector con promesas
function comprobarLector(valor){

    let miPromesa = new Promise((resolve, reject) => {
        let exito = true; // Simulación de éxito o error
        arrayLectores.forEach(lector => {

            //Si se encentra relacion y el lector no esta de baja
            if(lector.numSocio==valor && lector.bajaLector==false){
                exito=false;
            }
        });
          if (exito) {
            ...
            reject("Lector (inexistente o dado de baja)"); // Promesa rechazada
          }
      });
...
//FUNCION que genera el prestamo con promesas
function generarPrestamo(codLibro,numSocio){

    let miPromesa = new Promise((resolve, reject) => {
        let exito = true; // Simulación de éxito o error

         //Se resta un ejemplar
        let prestamo = new Prestamo(arrayTotalPrestamos.length+1, numSocio, codLibro, new Date().toLocaleDateString(), null); //Creamos el objeto Prestamo
...
if (exito) {
    ...
    reject("No se a podido generar correctamente"); // Promesa rechazada
  }
});
...
//FUNCION TOTAL para solicitar un prestamo asincrona.
async function solicitudPrestamo(numSocio,codLibro){
    
    try {
    const resultado1 = await comprobarLibro(codLibro);
    console.log(resultado1);
    
    const resultado2 = await comprobarLector(numSocio);
    console.log(resultado2);

    const resultado3 = await generarPrestamo(codLibro,numSocio);
...
//FUNCION para añadir un prestamo, evento "click" MODIFICADA
document.getElementById('prestamo-boton').addEventListener('click', () => {
    ...
    //Añadimos el mensaje debajo del boton
    document.getElementById('devolucion-prestamo').appendChild(mensajeDiv);
...
//------------CODIGO RESTANTE DE LA BIBLIOTECA------------//
